...
const server = http.createServer((req, res) => {
   ...
   * multipart/form-data
   */
  if (method === "POST" && !contentType.includes("application/octet-stream")) {
    let str = "";
...
/**
 * 通过路由处理请求数据的公共方法 callback带req
 */
async function processDateWithReq(
  method,
  pathname,
  query,
  post,
  ...
  // 若回调函数存在，则表示路由有配置相应的数据处理，即该请求不是获取静态文件。
    ...
    try {
      await callback(req, res, query, post, files, parame);
    } catch (error) {
      // 出现错误的处理
      res.writeHead(500);
      res.write("Internal Server Error", error);
