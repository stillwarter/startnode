...
<template>
  <div class="jsoopbox">
          ...
          <div>
            ...
            <div>
              ...
              *
              ...
              使得构造器看起来像一个类。构造器里的this指向的时构造函数的实例
              <br />
              *
    ...
    <div>
      ...
      <div>
        <h3>5.1 变量作用域与生存周期</h3>
        <div>
          变量作用域指变量可被访问和修改的范围，分为全局和局部两种类型。
          全局指的是在脚本任何地方可以被访问和修改，一般在函数外定义，使用var，let，const声明。另外声明的变量一般挂载在window全局对象上<br />
          局部指的是函数内声明的（但注意变量提升带来影响）
          <br />
          全局变量的生命周期从程序开始执行创建，到整个页面关闭时销毁；局部变量的生命周期从函数开始调用时创建，一直到函数调用结束。
        </div>
      </div>
